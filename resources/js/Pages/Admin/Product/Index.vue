...
<template>
  <div class="min-h-screen bg-gray-50 flex flex-col p-6">
	<div class="flex justify-between items-center mb-6">
	  ...
	  <Link href="/admin/products/create" class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded shadow">Add Product</Link>
	</div>
	  ...
	  <table class="table-auto w-full border-collapse">
		<thead>
		  <tr class="bg-gray-100 text-left">
			...
			<th class="px-6 py-2 text-gray-700">Category</th>
			<th class="px-6 py-2 text-gray-700">Price</th>
			<th class="px-6 py-2 text-gray-700">Stock</th>
		...
		<tbody>
		  <tr v-for="(product, index) in products" :key="product.id" class="hover:bg-gray-50">
			...
			<td class="px-6 py-4">{{ product.price != null ? (`$${Number(product.price).toFixed(2)}`) : '—' }}</td>
			<td class="px-6 py-4">{{ product.stock != null ? product.stock : '—' }}</td>
			<td class="px-6 py-4">
			  <div class="flex gap-2">
				<Link :href="`/admin/products/${product.id}/edit`" class="bg-green-500 hover:bg-green-600 text-white px-3 py-1 rounded shadow">Edit</Link>
				<Link :href="`/admin/products/${product.id}`" method="delete" as="button" class="bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded shadow">Delete</Link>
			  </div>
			</td>
		  </tr>
