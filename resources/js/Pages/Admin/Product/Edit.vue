...
const form = useForm({
  ...
  category_id: initial.category_id as string | number | null,
  price: initial.price as number | null,
        ...
        <header class="mb-6">
          <div class="flex items-start gap-4">
            ...
            <div>
              ...
              <p class="text-sm text-gray-500 mt-1">Update product details and manage variants.</p>
            </div>
          </div>
        </header>
        ...
        <form @submit.prevent="submitForm" novalidate>
          <div class="grid grid-cols-1 gap-8">
            ...
            <section>
              ...
              <div class="grid gap-6">
                <div>
                  ...
                  <div class="mt-1">
                    <input id="name" name="name" type="text" v-model="form.name" class="block w-full rounded-md border-gray-200 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" />
                  </div>
                ...
                <div>
                  ...
                  <div class="mt-1">
                    <textarea id="description" name="description" v-model="form.description" rows="4" class="block w-full rounded-md border-gray-200 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"></textarea>
                  </div>
            ...
            <section>
              ...
              <div class="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <div class="mt-1">
                    <input id="price" name="price" type="number" step="0.01" v-model.number="form.price" class="block w-full rounded-md border-gray-200 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" />
                  </div>
            ...
            <section>
              <div class="flex items-center justify-between mb-4 border-b pb-2">
                <div>
                  ...
                  <p class="text-sm text-gray-500">Manage variations with custom properties (e.g. Color, Size, Material).</p>
                </div>
              ...
              <div v-if="form.variants.length === 0" class="text-center py-10 bg-gray-50 rounded-2xl border-2 border-dashed border-gray-200">
                <p class="text-sm text-gray-400 font-medium">No variants created for this product.</p>
              </div>
                ...
                <div v-for="(variant, index) in form.variants" :key="index" class="p-6 bg-white border border-gray-200 rounded-2xl shadow-sm relative group hover:border-indigo-200 transition-all">
                  ...
                  <div class="flex items-center gap-2 mb-6">
                    ...
                    <h3 class="text-sm font-black text-gray-900 uppercase tracking-widest">
                        Variant Settings {{ variant.id ? '(ID: ' + variant.id.split('-')[0] + '...)' : '(New)' }}
                    </h3>
                  </div>
                  ...
                  <div class="grid grid-cols-2 gap-6 mb-8">
                    <div>
                      ...
                      <input type="number" step="0.01" v-model.number="variant.price" class="block w-full rounded-xl border-gray-200 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm transition-all" />
                    ...
                    <div>
                      ...
                      <input type="number" v-model.number="variant.stock" class="block w-full rounded-xl border-gray-200 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm transition-all" />
            ...
            <div class="flex items-center justify-end gap-3 pt-6 border-t border-gray-100">
              <button type="button" class="px-5 py-2.5 rounded-xl bg-white border border-gray-200 text-gray-700 font-bold text-sm hover:bg-gray-50 transition-all shadow-sm" @click.prevent="resetForm">Reset Changes</button>
