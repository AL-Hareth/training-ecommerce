...
const form = useForm({
  ...
  min_spend: props.voucher.min_spend ?? null,
  usage_limit: props.voucher.usage_limit ?? null,
  starts_at: props.voucher.starts_at ? props.voucher.starts_at.split('T')[0] : '',
  expires_at: props.voucher.expires_at ? props.voucher.expires_at.split('T')[0] : '',
  is_active: props.voucher.is_active,
})

function submitForm() {
  form.put(`/admin/vouchers/${props.voucher.id}`)
}
</script>
    ...
    <div class="w-full max-w-2xl">
      <div class="bg-white shadow-md rounded-lg p-8">
        <header class="mb-6 flex items-center gap-4">
          ...
          <h1 class="text-2xl font-semibold text-gray-800">Edit Voucher</h1>
        </header>
